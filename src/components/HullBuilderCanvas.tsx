...
export type HullBuildTool = "select" | "platform" | "ladder" | "erase";
...
interface HullBuilderCanvasProps {
  ...
  tool: HullBuildTool;
  onCell: (roomId: RoomId, target: GridCell) => void;
      ...
      >
        ...
        {rooms.map((room) => {
          ...
          return (
            <g
              key={room.id}
              className={`hull-blueprint-room ${room.structure === "core" ? "core" : ""} ${room.id === candidateRoomId ? "candidate" : ""}`}
          ...
          room.hardpoints.map((hardpoint) => {
            ...
            return (
              <g
                ...
                role={!occupied && committed ? "button" : undefined}
                tabIndex={!occupied && committed ? 0 : undefined}
                onClick={!occupied && committed ? activate : undefined}
                onKeyDown={
                  !occupied && committed ? (event) => activationKey(event, activate) : undefined
                }
              >
        ...
        {tool !== "select" &&
          rooms.flatMap((room) =>
            gridCellsForRoom(map, room.id).map((target) => {
              ...
              const activate = () => onCell(room.id, target);
              return (
                <g

...
const twoRouteRuntime = () => {
  ...
  const definition = deriveGame(DEFAULT_GAME_DEFINITION, {
    ...
    towers: {
      ...
      bolt_caster: {
        ...DEFAULT_GAME_DEFINITION.towers.bolt_caster,
        range: 100,
        firingArc: 360,
        lineOfSight: "lobbed",
      },
...
describe("deterministic tower combat", () => {
  ...
  it("applies direct attacks through damage ledgers, rewards, and source attribution", () => {
    ...
    expect(state.stats.damageBySource.tower_bolt).toBeGreaterThan(0);
    expect(state.stats.damageBySource.tower_repeater).toBeGreaterThan(0);

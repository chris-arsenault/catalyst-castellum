...
import { alignHullFragmentToMap, hullLayoutFromMap } from "./hullFragment";
import type { WorldMap } from "./map";
import { layoutHullAtAuthoredSite } from "./authoredHullLayout";

/**
 * Site materialization joins one authored campaign map to the player's persistent hull before an
 * operation begins. The simulation consumes the resulting map and does not compose topology.
 */
...
const siteWithoutHull = (spec: SiteSpec, hull: HullFragment): SiteSpec => {
  ...
  return {
    ...
    map: {
      ...spec.map,
      coreAnchor: {
        column: spec.map.coreAnchor.column - spec.hullAnchor.columns,
        elevation: spec.map.coreAnchor.elevation - spec.hullAnchor.elevations,
      },
      coreBreachCell: {
        column: spec.map.coreBreachCell.column - spec.hullAnchor.columns,
        elevation: spec.map.coreBreachCell.elevation - spec.hullAnchor.elevations,
      },
      rooms,
      connections,
      utilityNodes,
...
export const materializeAuthoredSite = (
  spec: SiteSpec,
  hull: HullFragment | null
): MaterializedSite => {
...
export const fixedSiteSpec = (definition: GameDefinition, levelId: LevelId): SiteSpec => ({
  map: {
    ...definition.map,
    connections: {
...
/**
 * One production entry point for scenario creation and docking against fixed campaign maps.
 */
export const materializeLevelSite = (
  ...
  if (!level.site)
    return materializeAuthoredSite(
      fixedSiteSpec(definition, levelId),
      incomingHull ? hull : incomingHull
    );
  return materializeAuthoredSite(
    { map: level.site.map, rounds: level.rounds, hullAnchor: level.site.hullAnchor },
    incomingHull ? hull : incomingHull
  );
};

...
test("places, aims, upgrades, and fires a free wall-mounted tower", async ({ page }) => {
  ...
  await page.getByTestId("tower-build-bolt_caster").click();
  await page.getByRole("button", { name: "Left wall" }).click();
  ...
  const point = await worldClientPoint(page, {
    x: room.bounds.column + 0.25,
    elevation: room.bounds.elevation + Math.min(2.5, room.bounds.height - 1),
  });
  await page.mouse.move(point.x, point.y);
  await page.mouse.click(point.x, point.y);

  await expect(page.getByText("Selected defense")).toBeVisible();
  await expect(page.getByRole("heading", { name: "Bolt Caster" })).toBeVisible();
  await page.getByTestId("tower-target-last").click();
  await page.getByTestId("tower-upgrade-bolt_calibration").click();
  await expect(page.getByTestId("tower-upgrade-bolt_calibration")).toContainText("Installed");
...
test("gives wall and ceiling projectors distinct vertical coverage", async ({ page }) => {
  await page.goto("/");
  ...
  await page.getByTestId("tower-build-line_projector").click();
  await page.getByRole("button", { name: "Left wall" }).click();
  await page.getByRole("button", { name: "right", exact: true }).click();
  const wallPoint = await worldClientPoint(
    page,
    { x: switchyard.bounds.column + 0.25, elevation: switchyard.bounds.elevation + 2.25 },
    harkerMap
  );
  ...
  await page.getByTestId("tower-build-line_projector").click();
  await page.getByRole("button", { name: "Ceiling" }).click();
